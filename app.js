...
// Approved widget types per lesson. R–Y2 deliberately restricted to
// addition/subtraction and PSHE widgets — never calculator, decimals or
// percentages, which do not appear on those lesson decks.
const LESSON_WIDGETS = {
  // Reception
  "R-1": ["maths-add", "pshe-wider"],
  "R-2": ["pshe-wider"],
  "R-3": ["maths-add", "pshe-wider"],
  "R-4": ["pshe-wider"],
  "R-5": ["pshe-wider"],
  "R-6": ["pshe-wider"],
  // Year 1
  ...
  "Y1-3": ["pshe-wider"],
  "Y1-4": ["pshe-wider"],
  "Y1-5": ["maths-add", "pshe-wider"],
  "Y1-6": ["pshe-wider", "pshe-relationships"],
  // Year 2
  "Y2-1": ["pshe-wider"],
  ...
  "Y4-2": ["pshe-wider"],
  "Y4-3": ["pshe-wider"],
  "Y4-4": ["pshe-wider", "computing-esafety"],
  "Y4-5": ["pshe-wider"],
  "Y4-6": ["pshe-wider", "maths-multiply", "maths-add"],
  // Year 5
  "Y5-1": ["maths-add"],
  "Y5-2": ["maths-multiply"],
  ...
  // Year 6
  "Y6-1": ["maths-percentages", "pshe-relationships", "pshe-wider"],
  "Y6-2": ["maths-percentages", "pshe-relationships", "pshe-wider"],
  "Y6-3": ["pshe-relationships", "computing-esafety"],
  "Y6-4": ["pshe-wider", "maths-add"],
  "Y6-5": ["maths-multiply", "maths-decimals", "maths-calculator"],
  "Y6-6": ["maths-decimals", "maths-percentages", "pshe-wider"]
};

// ─── LESSONS ──────────────────────────────────────────────────────────────────

// Full MoneySmarts programme: 42 lessons, 6 per year group, Reception–Year 6.
// Every lesson now has a matching recap text (all 42 complete).
const LESSONS = [
  // ── Phase 1 · EYFS (Reception) — 30-min story flip-book, sort/match only ──
  lesson("R-1",  "R",  1, "What does money look like?",        "Maths", ["early-number","coin-value"],          "Recognising coins and notes, noticing that money has different forms and values.", true),
  lesson("R-2",  "R",  2, "What is money for?",                "PSHE",  ["needs-wants","saving","helping"],     "Understanding that money lets us get things from others, and can be spent, saved or given.", true),
  lesson("R-3",  "R",  3, "How much do I have?",               "Maths", ["early-number","coin-value"],          "Counting small collections of coins and connecting numerals to value.", true),
  lesson("R-4",  "R",  4, "How should we use money?",          "PSHE",  ["needs-wants"],                         "Making simple spending choices and beginning to talk about fair exchange.", true),
  lesson("R-5",  "R",  5, "Why should we keep money safe?",    "PSHE",  ["safe-money"],                          "Looking after coins and choosing safer places to keep money.", true),
  lesson("R-6",  "R",  6, "Why do we save?",                   "PSHE",  ["saving","needs-wants"],               "Understanding that saving means keeping money to use later rather than spending it all now.", true),
  // ── Phase 1 · KS1 Year 1 — 45-min, minimal text (Jamal & Jasmine) ──
  lesson("Y1-1", "Y1", 1, "What are my coins worth?",          "Maths", ["coin-value","early-number"],          "Recognising and ordering coins up to £1, then using them to make simple amounts.", true),
  lesson("Y1-2", "Y1", 2, "How do I check my change?",         "Maths", ["coin-value","change"],                "Paying for small-priced items and checking simple change from 10p, 20p, 50p and £1.", true),
  lesson("Y1-3", "Y1", 3, "Do I need it or want it?",          "PSHE",  ["needs-wants"],                         "Sorting needs and wants and making choices when we cannot have everything.", true),
  lesson("Y1-4", "Y1", 4, "Why are banks the safest place?",   "PSHE",  ["safe-money"],                          "Comparing places to keep money and explaining why banks are safer than loose cash.", true),
  lesson("Y1-5", "Y1", 5, "Which is the better buy?",          "Maths", ["value","coin-value"],                 "Comparing two like-for-like options to see which gives more for the same money.", true),
  lesson("Y1-6", "Y1", 6, "Can I be kind with money?",         "PSHE",  ["helping"],                             "Using money to help or be kind to others, and recognising that giving is a choice.", true),
  // ── Phase 1 · KS1 Year 2 — 45-min (Fred, Fatima, Dev & Zia) ──
  lesson("Y2-1", "Y2", 1, "Where does money come from?",       "PSHE",  ["earning"],                             "Exploring earning, being given money, finding money and winning money.", true),
  lesson("Y2-2", "Y2", 2, "How do people pay for things?",     "Maths", ["payments","coin-value"],              "Comparing cash, cards and phones as ways to pay for goods and services.", true),
  lesson("Y2-3", "Y2", 3, "What happens if we don't keep track?", "Maths", ["records","change"],                "Using simple records to track spending and avoid running out of money.", true),
  lesson("Y2-4", "Y2", 4, "What happens when we save?",        "PSHE",  ["saving","needs-wants"],               "Saving over time, planning for a goal and recognising different spending choices.", true),
  lesson("Y2-5", "Y2", 5, "What tempts us to spend?",          "PSHE",  ["influence","saving"],                 "Recognising that adverts, shops and games persuade us to spend, tempting us off a saving goal.", true),
  lesson("Y2-6", "Y2", 6, "Where does the money go?",          "Maths", ["budgeting","records"],                "Seeing that a household shares money across many things, with the home taking the biggest share.", true),
  ...
  lesson("Y4-1", "Y4", 1, "How could my money help others?",   "PSHE",  ["helping"],                             "Using money to help others through giving, charity and community-minded choices.", true),
  lesson("Y4-2", "Y4", 2, "What are banks for?",               "PSHE",  ["safe-money","borrowing"],              "Understanding deposits, loans and the basic role of banks.", true),
  lesson("Y4-3", "Y4", 3, "Why does saving matter?",           "PSHE",  ["saving"],                              "Building the habit of saving and seeing why spending less than we earn matters.", true),
  lesson("Y4-4", "Y4", 4, "How do games & apps take your money?", "Computing", ["online-risk","influence"],      "Recognising digital spending traps, freemium design and in-app purchases.", true),
  lesson("Y4-5", "Y4", 5, "Why don't all jobs pay the same?",  "PSHE",  ["earning"],                             "Exploring pay, training, skills and the reasons jobs may be rewarded differently.", true),
  lesson("Y4-6", "Y4", 6, "Can you keep all you earn?",        "Maths", ["earning","records"],                   "Reading a payslip-style context and distinguishing gross pay from deductions.", true),
  // ── Phase 3 · Upper KS2 Year 5 ──
  lesson("Y5-1", "Y5", 1, "What's a budget and why do I need one?", "Maths", ["budgeting","records"],           "Creating a plan for income, spending and saving so money does not run out.", true),
  lesson("Y5-2", "Y5", 2, "How do I get the most from each pound?", "Maths", ["value","decimals"],              "Comparing value for money and making like-for-like spending decisions.", true),
  lesson("Y5-3", "Y5", 3, "How can I outsmart money tricksters?",   "Computing", ["online-risk"],               "Identifying scams, phishing and suspicious requests for money or information.", true),
  lesson("Y5-4", "Y5", 4, "What could persuade me to spend?",       "English",   ["influence"],                 "Evaluating advertising, peer pressure and emotional triggers to spend.", true),
  lesson("Y5-5", "Y5", 5, "What gambles are there in games?",       "Computing", ["online-risk","influence"],    "Understanding loot boxes, chance-based rewards and gambling-like mechanics.", true),
  lesson("Y5-6", "Y5", 6, "How does money work abroad?",            "Maths",     ["decimals","currency"],       "Using currency and exchange-rate contexts to compare money in different countries.", true),
  // ── Phase 3 · Upper KS2 Year 6 ──
  lesson("Y6-1", "Y6", 1, "What is the price of debt?",        "Maths", ["borrowing","percentages"],            "Calculating the cost of borrowing and seeing why repayments exceed the original loan.", true),
  lesson("Y6-2", "Y6", 2, "When is borrowing worth it?",       "PSHE",  ["borrowing","needs-wants"],            "Weighing up when borrowing is responsible and when it creates avoidable risk.", true),
  lesson("Y6-3", "Y6", 3, "'Easy money' — what's the catch?",  "Computing", ["online-risk"],                    "Recognising money muling and other risky shortcuts promoted online.", true),
  lesson("Y6-4", "Y6", 4, "What does 'wealth' really mean?",   "PSHE",  ["wealth","needs-wants"],               "Looking beyond income to assets, debt, wellbeing and subjective value.", true),
  lesson("Y6-5", "Y6", 5, "How can money earn more money?",    "Maths", ["saving","percentages"],               "Introducing interest, return and the idea that money can grow over time.", true),
  lesson("Y6-6", "Y6", 6, "How does investing work?",          "Maths", ["percentages","wealth"],               "Applying percentage growth and compounding to long-term investing contexts.", true)
];
...
const SCHEMES = {
  maths: {
    ...
    "big-maths": {
      name: "Big Maths (CLIC)",
      topics: {
        R:  topic("A2", 5, "Counting, Learn Its and early number fluency", ["early-number"]),
        Y1: topic("S2", 5, "CLIC progress drives: counting, number facts and coin contexts", ["coin-value","change","early-number"]),
        Y2: topic("A2", 5, "Money within CLIC and Beat That fluency; making amounts and change", ["coin-value","change","records"]),
        Y3: topic("S1", 5, "Column methods and applied money problem solving", ["records","payments"]),
        Y4: topic("S2", 5, "Decimals, measures and pounds-and-pence notation", ["decimals","earning"]),
        Y5: topic("S1", 5, "Multiplicative reasoning, decimals and value comparison", ["budgeting","value","decimals","currency"]),
        Y6: topic("S2", 5, "Percentages, ratio and applied financial reasoning", ["percentages","borrowing","wealth","saving"])
      }
    },
    numicon: {
      name: "Numicon (Oxford)",
      topics: {
        R:  topic("A2", 5, "Numicon shapes, pattern and early number sense", ["early-number"]),
        Y1: topic("S2", 5, "Structured apparatus for number bonds, then coin recognition", ["coin-value","early-number"]),
        Y2: topic("A2", 5, "Money, calculating and measures using structured apparatus", ["coin-value","change","records"]),
        Y3: topic("S1", 5, "Additive reasoning and money problem contexts", ["records","payments"]),
        Y4: topic("S2", 5, "Decimals, measures and money notation", ["decimals","earning"]),
        Y5: topic("S1", 5, "Multiplication, fractions and decimals for comparison", ["budgeting","value","decimals"]),
        Y6: topic("S1", 5, "Ratio, percentages and proportional reasoning", ["percentages","borrowing","wealth"])
      }
    },
    "inspire-maths": {
      name: "Inspire Maths (Oxford)",
      topics: {
        R:  topic("A2", 5, "Numbers to 10, comparing and early problem solving", ["early-number"]),
        Y1: topic("S2", 5, "Numbers to 100, addition/subtraction and money applications", ["coin-value","change"]),
        Y2: topic("A2", 5, "Money chapter: coins, notes, making amounts and change", ["coin-value","change","records"]),
        Y3: topic("S1", 5, "Addition/subtraction with bar-model money problems", ["records","payments"]),
        Y4: topic("S2", 5, "Decimals and measures applied to money", ["decimals","earning"]),
        Y5: topic("S1", 5, "Fractions, decimals and value-for-money comparison", ["budgeting","value","decimals"]),
        Y6: topic("S1", 5, "Percentages, ratio and multi-step financial problems", ["percentages","borrowing","wealth"])
      }
    },
    "oak-national": {
      name: "Oak National Academy",
      topics: {
        R:  topic("A2", 5, "Early number, counting and comparison units", ["early-number"]),
        Y1: topic("S2", 5, "Place value and additive structures, then money contexts", ["coin-value","change"]),
        Y2: topic("A2", 5, "Money unit: recognising coins, making amounts and finding change", ["coin-value","change","records"]),
        Y3: topic("S1", 5, "Addition and subtraction applied to money problems", ["records","payments"]),
        Y4: topic("S2", 5, "Decimals and measures including money notation", ["decimals","earning"]),
        Y5: topic("S1", 5, "Decimals, fractions and comparison for value", ["budgeting","value","decimals","currency"]),
        Y6: topic("S2", 5, "Percentages, ratio and financial problem solving", ["percentages","borrowing","wealth","saving"])
      }
    },
    "rising-stars": {
      name: "Rising Stars Mathematics",
      topics: {
        R:  topic("A2", 5, "Early number, sorting and comparing quantities", ["early-number"]),
        Y1: topic("S2", 5, "Number, place value and introductory money contexts", ["coin-value","change"]),
        Y2: topic("A2", 5, "Money and calculation within applied problem solving", ["coin-value","change","records"]),
        Y3: topic("S1", 5, "Written calculation and practical money problems", ["records","payments"]),
        Y4: topic("S2", 5, "Decimals, measures and money notation", ["decimals","earning"]),
        Y5: topic("S1", 5, "Decimals, fractions and value comparison", ["budgeting","value","decimals"]),
        Y6: topic("S2", 5, "Percentages and multi-step money problems", ["percentages","borrowing","wealth"])
      }
    }
  },
  pshe: {
    ...
    "story-project": {
      name: "The Story Project",
      topics: {
        // Book-led scheme: 6 units per year group, R–Y6. Economic wellbeing sits
        // within the 'Becoming a Global Citizen' key theme.
        R:  topic("S2", 4, "Becoming a Global Citizen: my surroundings, respect for others and the wider world", ["needs-wants","safe-money"]),
        Y1: topic("S2", 4, "Becoming a Global Citizen: online basics and learning about people different from ourselves", ["needs-wants","safe-money","helping"]),
        Y2: topic("S2", 4, "Becoming a Global Citizen: human rights, self-identity and thinking about value and money", ["saving","records","needs-wants","value","earning"]),
        Y3: topic("S2", 4, "Becoming a Global Citizen: internet dangers, acting for good and shared responsibilities", ["records","helping","influence","safe-money"]),
        Y4: topic("S2", 4, "Becoming a Global Citizen: acting for good, community and our own wellbeing", ["helping","influence","saving"]),
        Y5: topic("S2", 4, "Becoming a Global Citizen: online dangers, stereotypes and consumer awareness", ["online-risk","influence","value","borrowing"]),
        Y6: topic("S2", 4, "Becoming a Global Citizen: discrimination, career choices and the economic world", ["earning","wealth","saving","borrowing"])
      }
    },
    "ten-ten": {
      name: "Ten Ten – Life to the Full",
      topics: {
        R:  topic("U1", 4, "Created to Live in Community: our family, friends and looking after what we have", ["needs-wants","safe-money"]),
        Y1: topic("U1", 4, "Created to Live in Community: God's love in our community, sharing and helping", ["needs-wants","helping"]),
        Y2: topic("U1", 4, "Created to Live in Community: living in the wider world, sharing and stewardship", ["saving","helping","records"]),
        Y3: topic("U1", 4, "Created to Live in Community: community responsibility and caring for others", ["records","helping","safe-money"]),
        Y4: topic("U1", 4, "Created to Live in Community: giving, charity and the common good", ["helping","influence","saving"]),
        Y5: topic("U1", 4, "Created to Live in Community: online safety, influence and responsible choices", ["online-risk","influence","borrowing"]),
        Y6: topic("U1", 4, "Created to Live in Community: stewardship, work and social responsibility", ["earning","saving","wealth","helping"])
      }
    },
    heartsmart: {
      name: "HeartSmart",
      topics: {
        R:  topic("S1", 4, "Too Much Selfie Isn't Healthy: thinking of others and sharing", ["needs-wants","helping"]),
        Y1: topic("S1", 4, "Too Much Selfie Isn't Healthy: generosity, kindness and thinking of others", ["helping","needs-wants"]),
        Y2: topic("S1", 4, "Too Much Selfie Isn't Healthy / Fake Is a Mistake: giving and honesty", ["helping","saving","influence"]),
        Y3: topic("S1", 4, "Fake Is a Mistake: honesty, advertising and being true to yourself", ["influence","records"]),
        Y4: topic("S1", 4, "Too Much Selfie Isn't Healthy: generosity, charity and community", ["helping","influence","saving"]),
        Y5: topic("S1", 4, "Fake Is a Mistake: peer pressure, media influence and online honesty", ["influence","online-risk","value"]),
        Y6: topic("S1", 4, "No Way Through Isn't True: resilience, goals and looking ahead", ["earning","saving","wealth"])
      }
    },
    "my-happy-mind": {
      name: "myHappymind",
      topics: {
        R:  topic("U1", 4, "Engage: setting simple goals and understanding what I want", ["needs-wants"]),
        Y1: topic("U1", 4, "Engage: goal-setting, resilience and delaying gratification", ["needs-wants","saving"]),
        Y2: topic("U1", 4, "Engage: working towards a goal and sticking with it", ["saving","records"]),
        Y3: topic("U1", 4, "Appreciate / Engage: gratitude, goals and thinking about what matters", ["needs-wants","helping","records"]),
        Y4: topic("U1", 4, "Relate / Appreciate: helping others, gratitude and community", ["helping","influence","saving"]),
        Y5: topic("U1", 4, "Engage: long-term goals, resilience and resisting impulse", ["influence","value","online-risk"]),
        Y6: topic("U1", 4, "Engage: future goals, aspiration and planning ahead", ["earning","saving","wealth"])
      }
    },
    "life-lessons": {
      name: "Life Lessons",
      topics: {
        R:  topic("S2", 4, "Storytelling-led wellbeing: choices, belongings and looking after things", ["needs-wants","safe-money"]),
        Y1: topic("S2", 4, "Storytelling-led PSHE: needs, wants and keeping things safe", ["needs-wants","safe-money"]),
        Y2: topic("S2", 4, "Storytelling-led PSHE: saving, spending and making choices", ["saving","records"]),
        Y3: topic("S2", 4, "Storytelling-led PSHE: managing money, influence and responsibility", ["records","influence","safe-money"]),
        Y4: topic("S2", 4, "Storytelling-led PSHE: helping others and spending decisions", ["helping","influence","saving"]),
        Y5: topic("S2", 4, "Storytelling-led PSHE: persuasion, digital risk and value choices", ["influence","online-risk","value","borrowing"]),
        Y6: topic("S2", 4, "Storytelling-led PSHE: earning, saving and financial futures", ["earning","saving","wealth"])
      }
    },
    "picture-news": {
      name: "Picture News",
      topics: {
        R:  topic("A2", 4, "Weekly topical picture: fairness, choices and the world around us", ["needs-wants"]),
        Y1: topic("A2", 4, "Weekly topical picture: choices, fairness and responsibility", ["needs-wants","safe-money"]),
        Y2: topic("A2", 4, "Weekly topical picture: money in the news, saving and fair choices", ["saving","needs-wants","earning"]),
        Y3: topic("A2", 4, "Weekly topical picture: money, responsibility and critical thinking", ["records","influence","helping"]),
        Y4: topic("A2", 4, "Weekly topical picture: charity, fairness and spending decisions", ["helping","influence","earning"]),
        Y5: topic("A2", 4, "Weekly topical picture: advertising, digital risk and consumer choices", ["influence","online-risk","value"]),
        Y6: topic("A2", 4, "Weekly topical picture: the economy, inequality and financial choices", ["earning","wealth","borrowing","helping"])
      }
    }
  }
};
