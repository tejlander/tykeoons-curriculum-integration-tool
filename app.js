...
function buildOption(id, title, description, lessons, settings) {
  ...
  lessons.forEach((item, index) => {
    const placement = reserveSlot(placeLesson(item, settings, id, index), occupied, "lesson");
...
function placeLesson(item, settings, optionId, index) {
  ...
  if (optionId === "B") {
    ...
    if (Math.abs(topicIndex - halfTermIndex(spread.halfTerm)) <= 1) {
  ...
  const rationale = buildRationale(item, subject, sourceTopic, settings, optionId);
  return {
    year: item.year,
    subject,
...
function spreadSlot(item, index) {
  ...
  const halfTerm = HALF_TERMS[groupOrder % HALF_TERMS.length].id;
  const week = 3 + (Math.floor(groupOrder / 2) % 3);
  return { halfTerm, week };
}
...
function placeComprehension(item, placement) {
  ...
  return {
    ...
    rationale: item.comprehensionAvailable
      ...
      : `Reserve this reading-comprehension slot for the matching text once it is produced, keeping the same spaced-retrieval rhythm across the whole programme.`
  };
}

function buildRationale(item, subject, sourceTopic, settings, optionId) {
  ...
  return `${optionText} "${item.title}" fits ${subject} because ${item.focus.toLowerCase()} In ${schemeName}, this aligns with "${sourceTopic.title}", so it is placed ${timing}.${shift}`;
}
...
function normaliseSlot(halfTerm, week) {
  ...
  index = Math.max(0, Math.min(HALF_TERMS.length - 1, index));
...
function renderSummary(settings) {
  ...
  summaryCards.innerHTML = [
    ...
    [`${compCount}`, "spaced retrieval comprehension slots"],
...
function renderEvent(event) {
  ...
  const typeLabel = event.type === "comprehension" ? "Retrieval read" : "MoneySmarts lesson";
  return `
    <article class="event ${className}-bg ${event.type === "comprehension" ? "comprehension" : ""}">
      ...
      ${event.type === "comprehension" ? "<small>Two-week spaced retrieval revisit.</small>" : ""}
    </article>
  `;
}
